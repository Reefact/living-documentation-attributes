...
**324 patterns, 574 roles**, across 10 catalogs. A pattern is catalogued in the body of work that named it, under the name that work gave it — so a reader of a book finds its patterns spelled as it spelled them, and two works that name one pattern are related rather than merged.
...
| [ThreadSpecificStorage](#threadspecificstorage-pattern-oriented-software-architecture-volume-2) | Pattern-Oriented Software Architecture, Volume 2 | 3 | — |
...
`Reefact.LivingDocumentation.Attributes.Posa2` — 9 patterns, 30 roles.
...
| ActivationQueue | `[ActiveObject.ActivationQueue]` | interface, class | yes | `Proxy` |
...
**ActivationQueue** — Holds the method requests that are pending. It is bounded, which is the part that matters: it is what decouples the calling thread from the executing one, and what a client blocks on once the object is further behind than the queue is long.
...
| Role | Annotation | Applies to | Repeatable | Links |
|---|---|---|---|---|
| TSObjectProxy | `[ThreadSpecificStorage.TSObjectProxy]` | interface, class | no | — |
| TSObject | `[ThreadSpecificStorage.TSObject]` | field, property | no | `TSObjectProxy` |
| TSObjectCollection | `[ThreadSpecificStorage.TSObjectCollection]` | interface, class | yes | `TSObjectProxy` |

**TSObjectProxy** — The one access point every thread uses, which hides the key and the collection behind it and hands each caller its own object. It reads as global state and is not, which is the pattern's whole value and the reason a reader needs telling.

**TSObject** — One thread's own instance of the thread-specific object, reached only through the proxy. Nothing serializes access to it because nothing else can see it — so publishing a reference to it anywhere another thread can reach removes the only guarantee the pattern makes.

**TSObjectCollection** — The per-thread map from keys to that thread's objects, which the proxy uses and the caller never sees. On a platform whose runtime supplies thread-local storage this is the runtime's, and the role is for the codebases that keep their own.
