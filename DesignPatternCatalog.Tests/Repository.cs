...
namespace DesignPatternCatalog.Tests {
    ...
    internal static class Repository {
        ...
        public static Assembly[] CatalogueAssemblies { get; } =
            Catalogues.Select(name => Assembly.Load($"DesignPatternCatalog.{name}")).ToArray();

        private static string FindRoot() {
            DirectoryInfo? directory = new(AppContext.BaseDirectory);
            while (directory is not null && !File.Exists(Path.Combine(directory.FullName, "DesignPatternCatalog.sln"))) {
